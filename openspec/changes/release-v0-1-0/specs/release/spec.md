...
an explicit `include` list). `cargo package` MUST succeed, and the package
MUST build on non-Linux platforms (where the binary fails closed at
runtime, as the crate description states).
...
size, fixed theme, no wall-clock-dependent output in frame content.

#### Scenario: Regenerating the GIF
- **WHEN** `make demo-gif` runs on a machine with Docker
- **THEN** `demo/demo.gif` is produced from `demo/demo.tape` showing run → diff → undo with the files restored
